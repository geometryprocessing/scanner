...
from src.utils.image_utils import undistort_camera_points, homogeneous_coordinates
...
def triangulate_pixels(
    ...
    origin1, rays1 = camera_to_ray_world(pixels_1 + offset_1, R_1, T_1, K_1, dist_coeffs_1)
    origin2, rays2 = camera_to_ray_world(pixels_2 + offset_2, R_2, T_2, K_2, dist_coeffs_2)
    return intersect_line_with_line(origin1, rays1, origin2, rays2)
...
def intersect_pixels(
    ...
    _, rays_index_0 = camera_to_ray_world(
        lines_index_0.reshape((-1,2)) + offset_2,
        R_2,
        T_2,
        K_2,
        dist_coeffs_2)
    _, rays_index_1 = camera_to_ray_world(
    ...
    return intersect_line_with_plane(origin1, rays1, get_origin(R_2, T_2), normals)
...
def camera_to_ray_world(
    ...
    if R.shape!=(3,3):
        R, _ = cv2.Rodrigues(R)
    rays = camera_to_ray(points2D, K, dist_coeffs)
    origin = get_origin(R,T)
...
def camera_to_ray(
    ...
    rays = homogeneous_coordinates(uv)
...
def point_cloud_from_depth_map(
    ...
    result3D = homogeneous_coordinates(undistort_camera_points(campixels, K, dist_coeffs)) * depth_map[:, np.newaxis]
