...
# TODO: make K mean something here, esp for unit tests
class CameraConfig:
    def __init__(self):
...
class AtlasCamera(CameraConfig):
    def __init__(self):
        super.__init__()
...
class Camera:
    ...
    def load_calibration(self, calibration: str | dict | CameraConfig):
        # TODO: looks terrible, anything I can do instead?
        # maybe if string, check if name and get cam config,
        # otherwise see if path and load the json file
        # maybe if a dict, get cam config?
        if isinstance(calibration, str):
            calibration = load_json(calibration)
        if isinstance(calibration, CameraConfig):
        ...
        self.set_distortion(dist_coeffs)
        self.set_rotation(R)
